        ...
        <div class="panel panel-default summary">
            <div class="panel-body">
				...
				<div class="col-sm-12 col-md-12 cover_thumbnail_2" id="">
					<h3>Purchase Item</h3>
                	<hr/>
					<form method="post" id="submitFormPurchase">
						{{ csrf_field() }}
						...
						<div class="col-sm-3">
							<div class="form-group">
								<label>Quantity</label> <spand class="error qty"></spand>
								<input type="number" name="item_qty" id="item_qty" class="form-control" placeholder="0">
							</div>
						</div>
						<div class="col-sm-3">
							<div class="form-group">
								<label>Unit Price</label> <spand class="error price"></spand>
								<input type="number" name="unitprice" id="unitprice" class="form-control" placeholder="0">
							</div>
						</div>
						<div class="col-sm-3">
							<div class="form-group">
					        	<button type="button" name="submit_" class="btn btnadd btn-primary submit_" id="submit_" onclick="buttonClick();">Add Item</button>
					        	<a href="/purchasing" type="reset" class="btn btnadd btn-default">Back</a>
					        </div>
				        </div>
						<div class="col-md-12">
							<hr style="clear: both" />
								...
								<table class="table table-striped table-bordered">
									...
									<tbody id="showReturnInserted">
										@if(isset($purchaseID))
											...
											@foreach($getPurDetail as $detail)
												<tr>
												<input type="hidden" name="itemId[]" value="{{$detail->item_id}}">
						...
						<div class="row">
									...
									<select name="staffname" class="form-control" required>
										...
										@foreach(App\Tbl_staffs::all() as $staff)
											<option value="{{ $staff->id }}" 
												@if(isset($purchaseID[0]))
													@if($getPurDetail[0]->staff_id == $staff->id)
														selected
													@endif
												@endif
											>
							...
							<div class="col-sm-3">
								<div class="form-group">
									...
									<select name="supplier" id="supplier" class="form-control" required>
										...
										@foreach(App\Tbl_suppliers::all() as $supplier)
											<option value="{{$supplier->id}}"  
											@if(isset($purchaseID[0]))
												@if($getPurDetail[0]->supl_id == $supplier->id)
													selected
												@endif
											@endif
											>
									...
									<select name="zoneID" class="form-control" required>
										...
										@foreach(App\Tbl_zones::all() as $zone)
											<option value="{{ $zone->id }}" 
												@if(isset($getPurDetail[0]))
													@if($getPurDetail[0]->zone_id == $zone->id)
														selected
													@endif
												@endif
											>
							...
							<div class="col-sm-3">
								<div class="form-group">
									...
									<input type="text" name="datePurchase" id="datePurchase" class="show_current_date form-control" required 
									value="@if(isset($purchaseID)){{ date('d-M-Y', strtotime(isset($getPurDetail[0])?$getPurDetail[0]->purchase_date:'')) }} @endif" placeholder="Date Request">
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
									...
									placeholder="Note">{{isset($getPurDetail[0])?$getPurDetail[0]->note:'' }}</textarea>
								</div>
							</div>
						</div>
						<div class="row">
	...
	//=======================start function================
	    ...
	    function buttonClick() {
	    	...
	    	if(item_id && item_qty && unit_price){
		    	$.get("/storeItemPurchase/"+item_id, function(data, status){
		            //alert("Data: " + data + "\nStatus: " + status);
				...
				if(item_id==""){
					$('.item').text('Item name is required!');
				}else {
					 $('.item').text('');
				}
				if(item_qty==""){
					$('.qty').text('Qty is required!');
				}else{
					 $('.qty').text('');
				} 
				if(unit_price==""){
					$('.price').text('Unit Price is required!');
				}else{
					 $('.price').text('');
				}
			}
		...
		$(function () {
	        ...
	        $(document).on('submit', "#submitFormPurchase",function (e) {
			 	e.preventDefault();
				...
				$.ajax({
					...
					processData: false,
					contentType: false,
					url: "/addItemToPurchasing",
					data: formData,
					success: function (response) {
						//alert(response);
						if(response!='no'){
							swal({
	                            title:"Purchase Item Success!",
	                            text:"Purchase Item Success!",
	                            type:"success",  
	                            timer: 2000,   
